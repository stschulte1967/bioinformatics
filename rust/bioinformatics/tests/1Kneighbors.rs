...
fn test_neighbors(input_file: &str) {
    let input_params = read_parameters_from_file("../../data/1K/inputs/".to_string() + input_file);
    let output_params = read_parameters_from_file("../../data/1K/outputs/".to_string() + input_file);
    let result = neighbors(&input_params[0], input_params[1].parse().unwrap());
    println!("{:?}", result);
...
#[test]
fn test1_neighbors() {
    test_neighbors("testset.txt");
}

#[test]
fn test2_neighbors() {
    test_neighbors("dataset_30282_4.txt");
}

#[test]
fn test3_neighbors() {
    test_neighbors("quiz.txt");
}