use bioinformatics::{randomized_motif_search, score};
use std::fs::File;
...
fn test_randomized_motif_search(input_file: &str) {
    let input_params = read_parameters_from_file("../../data/2F/inputs/".to_string() + input_file);
    let output_params = read_parameters_from_file("../../data/2F/outputs/".to_string() + input_file);
    
    let mut best_score = 10000;
    let mut best_motif = Vec::new();
    for i in 0..1000 {
        let current_motifs = randomized_motif_search(input_params[2..].to_vec(), input_params[0].parse().unwrap(), input_params[1].parse().unwrap());
...
fn test4_randomized_motif_search() {
    test_randomized_motif_search("dataset_30307_5.txt");
}
